...
function Register() {
  ...
  const handleSave = async () => {
    try {
      ...
      navigate('/business/login'); // 성공 페이지로 이동
    } catch (error) {
    ...
    <div className='mid' lang='ko'>
      ...
      <div className='main-mid'>
        ...
        <div className='input-container-id'>
          <p>아이디</p>
          <input type='text' name='login_id' value={formData.login_id} onChange={handleInputChange} placeholder='아이디를 입력해 주세요' />
          <button className='checkLogin' onClick={checkLogin}>아이디 중복확인</button>
        </div>
        <div className='input-container'>
          <p>비밀번호</p>

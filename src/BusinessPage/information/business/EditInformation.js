...
function EditInformation() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get("/api/business/information/edit", {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.put(
        `${apiUrl}/api/business/edit/information`,
        data,
        {
          headers: {
  ...
  return (
    <div className="mid" lang="ko">
      ...
      <div className="main-mid">
        ...
        <div className="input-container">
          <button onClick={handleAddressSearch}>가게주소(찾기)</button>
        </div>
        <div className="input-container">
          <p>우편번호</p>
          <input

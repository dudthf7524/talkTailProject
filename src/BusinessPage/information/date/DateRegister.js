...
function DateRegister() {
  ...
  const handleSave = async () => {
    ...
    try {
      await api.post("/api/business/date/register", transformedDateList, {
  ...
  return (
    <div className="page-container" lang="ko">
      <div className="navigation">
        ...
        {step === 1 && <span>영업일&휴무일</span>}

        {step === 2 && <span>시간 설정</span>}

        {step === 1 && (
          <div style={{ cursor: "pointer" }} onClick={handleGoToTimeSelection}>
            시간설정
          </div>
        )}

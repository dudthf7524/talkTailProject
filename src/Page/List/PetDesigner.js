...
const PetDesigner = () => {
  ...
  const business_registration_number =
    designerName.businessInfo.business_registration_number;
    ...
    const fetchBusiness = async () => {
      try {
        ...
        const response = await api.get(
          `/api/designer/list/${business_registration_number}`,
          {
            headers: {
  ...
  return (
    <div lang="ko">
      ...
      <div className="pet_list_box">
        {Array.isArray(designers) &&
          designers.map((designer, index) => (
            <div key={index} className="pet_list">
              <div className="desinger_ngi_img">
                <img
                  style={{ width: "100px", height: "100px" }}
                  src={`${process.env.PUBLIC_URL}/profile/${designer.business_desinger_profile}`}
                ></img>
                <div className="desinger_ngi">
                  <div className="desinger_ng">
                    <div className="desinger_name">
                      {designer.business_desinger_name}
                    </div>
                    <div className="desinger_g">
                      <div className="desinger_grade">
                        {designer.business_desinger_grade}
                      </div>
                    </div>
                  </div>
                  <div className="desinger_introduce">
                    {designer.business_desinger_introduce}
                  </div>
                </div>
              </div>

  ...
  try {
    const BeautyReservationData = await BeautyReservation.create({
      business_registration_number:
        beautyReservationData.business_registration_number,
      ...
      reservation_applicationTime:
        beautyReservationData.reservationApplicationTime,
...
const beautyReservationGet = async (business_registration_number) => {
  ...
  try {
     ...
     sql += "left join user_information ui ";
...
const beautyReservationDetail = async (id) => {
  try {
    ...
    sql +=
      "on tbd.business_registration_number = bi.business_registration_number ";
    sql += "join store_hours sh ";
    sql +=
      "on br.business_registration_number = sh.business_registration_number ";
    ...
    const [results, metadata] = await sequelize.query(
      ...
      {
        replacements: { id: Number(id) }, // 바인딩 파라미터
    ...
    const calculateAge = (birthDate, referenceDate = dayjs()) => {
      ...
      return referenceDate.isBefore(adjustedReference)
        ? yearsDiff - 1
        : yearsDiff;
    };
...
const beautyReservationReject = async (
...
const reservationBookmark = async (platform_id) => {
  ...
  try {
    let sql = "";
    sql +=
      "select business_name, business_main_image, count(business_name) as user_count from beauty_reservation br ";
    sql += "join business_information bi ";
    sql +=
      "on br.business_registration_number = bi.business_registration_number ";
    sql +=
      "where br.reservation_state='완료' and br.platform_id = :platform_id ";
